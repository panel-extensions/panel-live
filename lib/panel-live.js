...
function renderError(targetEl, errorMsg) {
  const escaped = (errorMsg || '').replace(/</g, '&lt;');
  const lines = escaped.split('\n');
  const lastLine = lines[lines.length - 1] || escaped;

  targetEl.innerHTML = `<div class="pl-error-panel">
    <div class="pl-error-header">${lastLine}</div>
...
__exec_result__ = exec_with_return(__panel_user_code__, __ns__, stderr=__stderr_buf__)
__exec_error__ = __stderr_buf__.getvalue()
if __exec_result__ is None and __exec_error__:
    raise RuntimeError(__exec_error__)
`);
      await loadExtensionResources();
      await _pyodide.runPythonAsync(`
import js
...
`);
    ...
    renderError(targetEl, error.message || String(error));
...
function resolveTheme(themeAttr) {
  ...
  const mdScheme = document.body && document.body.dataset.mdColorScheme;
  ...
  _renderEditorMode(rt, layout) {
    ...
    if (codeVis === 'hidden') {
      ...
      container.innerHTML = `
        ...
        <div class="pl-code-toggle"><button title="Toggle code visibility">&lt;&gt; Code</button></div>
        <div class="pl-output-wrapper">
          <div class="pl-status"><span class="pl-spinner"></span> Initializing...</div>
          <div class="pl-output" id="${outputId}"></div>
        </div>
    ...
    if (toggleBtn) {
      ...
      toggleBtn.addEventListener('click', () => {
        ...
        if (isCollapsed) {
          ...
          if (this._cm) setTimeout(() => this._cm.refresh(), 350);
          codeSection.addEventListener('transitionend', () => {
...
`);
  ...
  async _runFromEditor(textarea, outputEl, statusEl) {
    ...
    // Ensure Pyodide is initialized (needed when auto-run is disabled)
    await initPyodide(msg => { statusEl.textContent = msg; });

    if (this._requirements) {
      await installExplicitRequirements(this._requirements, msg => { statusEl.textContent = msg; });
    }
