...
function _validateMainMessage(msg) {
  const validTypes = ['init', 'run', 'install', 'write-file', 'rendered', 'patch', 'reset', 'eval', 'server-data'];
  ...
  try {
    switch (msg.type) {
      ...
      case 'eval':
        ...
        break;
      case 'server-data':
        await enqueue(() => handleServerData(msg));
        break;
      default:
...
`);
    ...
    // Output callback: user Python code calls send_output(data) → this JS global
    pyodide.globals.set('__send_output_raw__', (jsonStr, targetId) => {
      self.postMessage({ type: 'output', targetId, data: JSON.parse(jsonStr) });
    });

    // Initialize reactive server data: Parameterized class + per-element dict
    await pyodide.runPythonAsync(`
...
// --- Server data: receive data from server and store as Python global ---

async function handleServerData(msg) {
  const { targetId, data } = msg;
  if (!initPromise) return;
  await initPromise;
  // JSON round-trip to ensure native Python types (not JsProxy)
  pyodide.globals.set('__server_data_json__', JSON.stringify(data));
  pyodide.globals.set('__server_data_target_id__', targetId);
  await pyodide.runPythonAsync(`
import json as _j
