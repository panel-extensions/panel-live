...
class WorkerBridge {
  ...
  _resetStallTimer() {
    ...
    // Only run stall detection while init or runs are pending
    // Note: check _initResolve too — _initPromise may not be assigned yet
    // when called from inside _doInit (async timing)
    if (!this._initPromise && !this._initResolve && Object.keys(this._runs).length === 0) return;
  ...
  _attemptRecovery() {
    ...
    // Reject all pending runs (they can't be replayed)
  ...
  _validateWorkerMessage(msg) {
    const validTypes = ['ready', 'status', 'render', 'no-output', 'stdout', 'stderr', 'patch', 'idle', 'error', 'done'];
