...
// Shadow DOM support: when <panel-live> is inside a Shadow DOM (e.g. inside a Panel
// Markdown or HTML pane), patch document.getElementById so Bokeh's embed_items()
// can still find the #el-XXXX output containers, and inject CSS so the component
// renders correctly inside the shadow root.
const _plShadowRoots = new Set();
let _plGetByIdPatched = false;

function _patchGetElementByIdForPL(shadowRoot) {
  _plShadowRoots.add(shadowRoot);
  if (_plGetByIdPatched) return;
  _plGetByIdPatched = true;
  const _origGetById = document.getElementById.bind(document);
  document.getElementById = function (id) {
    const result = _origGetById(id);
    if (result) return result;
    for (const root of _plShadowRoots) {
...
function _injectPanelLiveCSSIntoShadowRoot(shadowRoot) {
  // Clone the panel-live <link> from <head> (loaded via pn.extension css_files)
  for (const link of document.head.querySelectorAll('link[rel="stylesheet"]')) {
    if (link.href && /panel-live(?:\.min)?\.css/.test(link.href)) {
      shadowRoot.appendChild(link.cloneNode(true));
      return;
    }
  }
  // Fallback: derive CSS URL from the panel-live.js <script> src
  for (const s of document.querySelectorAll('script[src*="panel-live"]')) {
    if (/panel-live(?:\.min)?\.js/.test(s.src)) {
      const cssUrl = s.src.replace(/panel-live(?:\.min)?\.js(\?.*)?$/, 'panel-live.css$1');
...
class PanelLive extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // If inside a Shadow DOM, patch getElementById and inject CSS so Bokeh rendering works
    const rootNode = this.getRootNode();
    if (rootNode instanceof ShadowRoot) {
      _patchGetElementByIdForPL(rootNode);
      _injectPanelLiveCSSIntoShadowRoot(rootNode);
    }
    if (!this._rendered) {
