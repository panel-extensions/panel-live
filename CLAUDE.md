...
- **`cdn_upload.py`** — secondary/backup: uploads `dist/` JS/CSS to `s3://cdn.holoviz.org/panel-live/{version}/` and `latest/`. Primary distribution is via npm (served by jsDelivr at `cdn.jsdelivr.net/npm/@panel-extensions/panel-live@latest/dist/`).
